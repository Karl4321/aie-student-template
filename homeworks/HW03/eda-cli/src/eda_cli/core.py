...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    sum_cardinality_categoricals = 0 
    
    for i in summary.columns:
        sum_cardinality_categoricals+=int(i.unique>10)

    flags["has_high_cardinality_categoricals"] = bool(sum_cardinality_categoricals>6)
    flags["has_suspicious_id_duplicates"] = bool(summary.columns[0].unique == summary.n_rows-1)
