...
- **Признаки**: Все 60 признаков (`f01`–`f60`) являются числовыми вещественными значениями. Пропусков данных нет.
...
- **Подбор гиперпараметров**:
  ...
  - Исследованы параметры: `n_estimators` [100, 300, 500], `max_depth` [3, 5, 7]
- **Метрики**:
...
2. **LogisticRegression (baseline)**:
   - L2-регуляризация
   - Балансировка весов классов для учета дисбаланса
   - Стандартизация признаков через Pipeline
3. **DecisionTreeClassifier**:
   - `criterion="gini"`, `max_depth=16`, `min_samples_leaf=10`
   - Веса классов для учета дисбаланса
   - Контроль сложности через ограничение глубины и минимального количества образцов в листе
4. **RandomForestClassifier**:
   - `n_estimators=1000`, `criterion="gini"`, `max_depth=16`
...
- **Precision**: 0.857 (доля верно предсказанных положительных классов)
- **Recall/Sensitivity**: 0.860 (доля верно найденных положительных классов)
- **Specificity**: 0.993 (доля верно найденных отрицательных классов)

**Комментарий**:
- Модель отлично справляется с классом 0 (specificity=0.993)
- Для миноритарного класса 1 показатели ниже, но всё же хорошие (precision=0.857, recall=0.860)
...
1. **Ансамбли превосходят одиночные модели**: StackingClassifier показал на 4-6% лучший ROC-AUC по сравнению с одиночными деревьями и на 14% лучше логистической регрессии. Композиция моделей позволяет использовать сильные стороны каждой из них.

2. **Дисбаланс требует специальной обработки**: Использование `class_weight='balanced'` или ручной балансировки весов классов значительно улучшило метрики для миноритарного класса (F1 вырос с 0.18 до 0.86). Без этого модели показывали высокую accuracy, но плохо предсказывали класс 1.

3. **Сложность vs производительность**: Deep Decision Trees (max_depth=16) показали хорошие результаты (ROC-AUC=0.90), но RandomForest с аналогичной глубиной оказался лучше (ROC-AUC=0.98) благодаря бэггингу, который снижает дисперсию.

4. **Stacking эффективен, но дорог**: Композиция моделей дала лучший результат, но требует в 3-4 раза больше вычислительных ресурсов и времени обучения. Для production-систем необходимо оценивать компромисс между качеством и ресурсами.

5. **Гиперпараметры критически важны**: GradientBoosting с подобранными параметрами (GridSearchCV) показал результат близкий к RandomForest, но без подбора был значительно хуже. Правильная настройка гиперпараметров может дать прирост 5-10% по ROC-AUC.

6. **Честный протокол требует разделения данных до любой обработки**: Стратификация при разбиении и использование отдельной тестовой выборки (ни разу не использованной при обучении) - обязательные условия для получения достоверных оценок. GridSearchCV должен выполняться только на тренировочной части данных, а финальная оценка - только на тестовой.